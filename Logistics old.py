    ...
    try:
        for key in stop_list:
            if day==stop_list[key][1]+1:
                ...
                del stop_list[key]
                print(f'транспорт {key} осуществил доставку, список занятых {stop_list} , список свободных {Auto_transport.d_city}')
    except RuntimeError:
        ...
        if len(request.split( ))==5:
            ...
            try:
                ...
                if Auto_transport.d_city[key][0]==leave_city and float(Auto_transport.d_city[key][1])>=float(request.split( )[2]) and\
                    ...
                    if Auto_transport.d_city[key][3] == 0 and float(wight) <(Auto_transport.d_city[key][1])*0.8 and \
                        leave_city in Airplane_transport.airport and destination in Airplane_transport.airport:
                        ...
                        if m==1:
                            ...
                            if groupage_cargo!={}:
                                print('groupage_cargo[key][2]=',key, groupage_cargo[key])
                                print('destination=', destination)
                            if key in groupage_cargo and groupage_cargo[key][2]==destination:
                                summa_wight=0
                                a = groupage_cargo[key]
                                print('a=', a)
                                groupage_cargo.update({key:(a, (name, float(wight), destination))})
                                print('groupage_cargo', groupage_cargo)
                                for el in key[0]:
                                    print('el=',el)
                                    summa_wight+=el[1]
                                ...
                                groupage_cargo.update({key:(name,float(wight),destination)})
                            print('shipment has been added to the waiting list', groupage_cargo)

                    elif Auto_transport.d_city[key][3] == 0 and key in groupage_cargo and groupage_cargo[key][2]==destination:
                        print('need to wait for cargo')
                        m=int(input("you need to choose: 1 - I'm waiting for a groupage cargo 2 - I need another option:"))
                        if m==1:
                            count += 1
                            ...
                            print('a=', a)
                            groupage_cargo.update({key:(a, (name, float(wight), destination))})
                            print('groupage_cargo gggggggggggggggg',groupage_cargo)
                            ...
                            #         stop_list.update({key:(Auto_transport.d_city[key],day)})
                            #         Auto_transport.del_transport(key)
                            #         print('stop_list= ', stop_list)
                            #         print('Auto_transport.d_city= ',Auto_transport.d_city)
                            #
                            #
                            #
                    ...
                    elif Auto_transport.d_city[key][3] != 0:
